...
if __name__ == '__main__':
    ...
    sub_all = list(range(11, 12))
    subs = list(range(1,55))
    ...
    for sub in sub_all:
        ...
        x = tra_model.output
        x = tf.keras.layers.Dense(128, activation='elu', name='Dense_relevant_top_1')(x)  # 39
        x = tf.keras.layers.Dense(64, activation='elu', name='Dense_relevant_top_2')(x)
        x = tf.keras.layers.Dense(32, activation='elu', name='Dense_relevant_top_3')(x)
        ...
        tra_model.summary()
        t_data= get_all_data_OpenBMI_inter(yml, channDic, sub)
        a = t_data['x']
        mean = a.mean()
        ...
        #     s_data= get_all_data_OpenBMI_inter(yml, channDic, idx)
        ...
        #     tra_model.fit(x=s_data['x'], y=s_data['y'],
        #                   shuffle=yml['ML']['shuffle'],
        #                   batch_size=yml['ML']['batch_size'],
        #                   epochs=yml['ML']['trainEpoch'],
        #                   class_weight=class_weight_f(yml, s_data['y']),
        #                   callbacks=callback)
        ...
        #     # x = tra_model.output
        #     # x = tf.keras.layers.Dense(16, activation='elu', name='Dense_Spe1_{}'.format(idx))(x)  # 40
        #     # x = tf.keras.layers.Dense(16, activation='elu', name='Dense_Spe2_{}'.format(idx))(x)  # 41
        ...
        #     mmd_model.summary()
        #     soure_data = mmd_model.predict(x=s_data['x'])
        #     mmd_model.compile(loss=MMD,
        #                       optimizer=Nadam(learning_rate=yml['ML']['learningRate'], beta_1=0.9, beta_2=0.999))
        #     mmd_model.fit(x=t_data['x'], y=soure_data,
        #                   shuffle=yml['ML']['shuffle'],
        #                   batch_size=yml['ML']['batch_size'],
        #                   epochs=yml['ML']['trainEpoch'],
        #                   callbacks=callback)
        ...
        writerxlsx_1(
            path=os.path.join(best_dir, 'result_OpenBMI_inter_step1.xlsx'),
