...
def load_filter_data(load_path,sub,sess):
    try:
        file_raw_path = load_path + '/S{:03d}_sess{:03d}_Raw.npy'.format(sub, sess)
        file_event_path = load_path + '/S{:03d}_sess{:03d}_Events.npy'.format(sub, sess)
        file_raw = np.load(file_raw_path, allow_pickle=True)
        file_event = np.load(file_event_path, allow_pickle=True)

        print('RawData shape{},Events shape{}'.format(file_raw.shape, file_event.shape))
    except:
        raise Exception(
            'Path Error: file does not exist, please check this path {}, and {}'.format(file_raw_path, file_event_path))
    return file_raw, file_event
    ...
    if TrainType == 'dependent':
        ...
        if num_class == 3:
            ...
            for subject in n_subjs:
                ...
                for id_se, sess in enumerate(sessions):
                    select_event= []
                    RawData, EventData= load_filter_data(PATH, subject, sess)
                    # 1.选择对应的events: 0=rest,1=left,2=right
                    rest_Index = np.where(EventData[:, 2] == 0)[0]
                    left_Index = np.where(EventData[:, 2] == 1)[0]
                    right_Index = np.where(EventData[:, 2] == 2)[0]
                    event_Index = np.sort(np.concatenate((left_Index, right_Index,rest_Index)))
                    for i in event_Index:
                        select_event.append(EventData[i])
                    select_event = np.array(select_event, dtype='int32')
                    # 2.切割数据
                    Data, Label = getData_chanSeq_OpenBMI(yml, RawData, select_event, chanDic,
                                                                        eventTypeDic=eventTypeDic, getAll=False)
                    Data, Label = shuffle(Data, Label)

                    # （700*4,31,1000）
                    x_session.extend(Data)
                    y_session.extend(Label)
