...
def load_raw(dataset):
    ...
    folder_name = '/media/oymotionai/DATA/EEG/MI/DataSet/OpenBMI'
    if dataset == 'OpenBMI':
        try:
            num_subjects = 54
            sessions = [1, 2]
            ...
            for session in sessions:
                for person in range(1, num_subjects+1):
                    ...
                    #datasets/OpenBMI/raw/sess01_subj02_EEG_MI.mat
...
class DataLoader:
    def __init__(self, dataset, train_type=None, data_type=None, num_class=2, subject=None, data_format=None, dataset_path='/datasets', **kwargs):
        ...
        #datasets/OpenBMI/time_domain/2_class/subject_independent
        self.path = self.dataset_path+'/'+self.dataset+'/'+self.data_type+'/'+str(self.num_class)+'_class/'+self.train_type
        # self.path = self.dataset_path + '/' + self.dataset + '/' + self.data_type + '/' + 'noresample/'+ self.train_type
    def _change_data_format(self, X):
        if self.data_format == 'NCTD':
            ...
            # (#被试个数, #trail 实验次数, #channels, #depth) x(42*2*200,20,400)==>x(42*2*200,20,400,1)
            X = X.reshape(X.shape[0], X.shape[1], X.shape[2], 1)
            #x(42*2*200,1,400,20)
            X = np.swapaxes(X, 1, 3)
    ...
    def load_train_set(self, fold, **kwargs):
        ...
        try:
            # datasets/OpenBMI/time_domain/2_class/subject_independent/X_train_S001_fold001.npy file_x(42*200*2,20,400)
    ...
    def load_val_set(self, fold, **kwargs):
        ...
        X, y =  np.array([]),  np.array([])
        try:
            # datasets/OpenBMI/time_domain/2_class/subject_independent/X_val_S001_fold001.npy
    ...
    def load_test_set(self, fold, **kwargs):
        ...
        X, y =  np.array([]),  np.array([])
        try:
            # datasets/OpenBMI/time_domain/2_class/subject_independent/X_test_S001_fold001.npy
...
def butter_bandpass_filter(data, lowcut, highcut, fs, order):
    nyq = 0.5 * fs #100
    low = lowcut / nyq  #8 0.16
    high = highcut / nyq #30 0.6
