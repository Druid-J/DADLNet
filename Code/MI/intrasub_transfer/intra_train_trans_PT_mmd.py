...
import csv
from MI.intrasub_transfer.load_data_2 import get_all_data
from MI.intrasub_transfer.load_data_new import get_data
...
from MI.intrasub_transfer.tool import writerxlsx_1
...
if __name__ == '__main__':
    ...
    for sub in sub_all:
        ...
        modelNet.envalue(te_dataset['x'],te_dataset['y'])
        # 2.设置模型的可训练程度
        ...
        #为了训练新加入的Dense层 通过loss函数判断 如果是为了微调
        ...
        tra_model.fit(x=train_data['tra2'], y=train_data['tra2_label'],
                     ...
                     callbacks=callback)
        t_model = M(inputs=tra_model.input, outputs=tra_model.get_layer(index=40).output)
        t_model = reset_model(1, t_model)#设置最后两层可训练，使用最后两层以及session1的train1和session2的train2进行相关性计算
        ...
        t_model.compile(loss=MMD,
                          optimizer=Nadam(learning_rate=yml['ML']['learningRate'], beta_1=0.9, beta_2=0.999))
        print('fit mmd te1')
        t_model.fit(x=test_data['te1'], y=soure_data_2,
                      shuffle=yml['ML']['shuffle'],
                      batch_size=yml['ML']['batch_size'],
                      epochs=yml['ML']['trainEpoch'],
                      callbacks=callback)
        print('fit mmd te2')
        t_model.fit(x=test_data['te2'], y=soure_data_1,
                      shuffle=yml['ML']['shuffle'],
                      batch_size=yml['ML']['batch_size'],
                      epochs=yml['ML']['trainEpoch'],
                      callbacks=callback)
        ...
        sigmoid = modelNet.model.layers[-1]#基础模型的最后一层
        y = t_model.output
        y = dense(y)
        y = sigmoid(y)
        ...
        for tra_index, te_index in kf.split(X):
            ...
            if not os.path.exists(transfer_best_path):
                ...
                te_model.save(os.path.join(transfer_best_path, 'transfer_Dense_2_400Hz_mmd_4.h5'))
            else:
                te_model.save(os.path.join(transfer_best_path, 'transfer_Dense_2_400Hz_mmd_4.h5'))
        ...
        writerxlsx_1(
            path=os.path.join(best_dir, 'val_Dense_400Hz_2_mmd_4.xlsx'),
    ...
    writerxlsx_1(
        path=os.path.join('/home/xumeiyan/Public/Reslut/CMANet/Try/202201201_base3DCNN_OpenBMI_400Hz_2class_4_9channel_0.0625_Combined_Attention_1Layer/result',
                          'transfer_Dense_2_val_400Hz_mmd_4.xlsx'),
