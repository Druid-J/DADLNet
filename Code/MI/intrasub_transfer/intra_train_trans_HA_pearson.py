...
def pearson(y_true, y_pred):
    ...
    res = tf.math.abs(res)
    pearson_loss = 1 / tf.nn.moments(res, axes=[0])[0]
...
if __name__ == '__main__':
    ...
    for sub in sub_all:
        ...
        tra_model = reset_model(2, tra_model)  # index=-4
        ...
        tra_model.fit(x=train_data['tra2'], y=train_data['tra2_label'],
                     ...
                     callbacks=callback)
        # pearson_loss&mmd loss
        t_model_1 = M(inputs=tra_model.input, outputs=tra_model.get_layer(index=39).output)
        t_model_1 = reset_model(1, t_model_1)
        soure_data_1 = t_model_1.predict(x=train_data['tra1'])
        soure_data_2 = t_model_1.predict(x=train_data['tra2'])
        t_model_1.compile(loss=pearson,
                          optimizer=Nadam(learning_rate=yml['ML']['learningRate'], beta_1=0.9, beta_2=0.999))
        print('--------------- 1 fit pearson te1---------------')
        t_model_1.fit(x=test_data['te1'], y=soure_data_2,
                      shuffle=yml['ML']['shuffle'],
                      batch_size=yml['ML']['batch_size'],
                      epochs=yml['ML']['trainEpoch'],
                      callbacks=callback)
        print('--------------- 1 fit pearson te2---------------')
        t_model_1.fit(x=test_data['te2'], y=soure_data_1,
                      shuffle=yml['ML']['shuffle'],
                      batch_size=yml['ML']['batch_size'],
                      epochs=yml['ML']['trainEpoch'],
                      callbacks=callback)
        n_layer = tra_model.layers[-3]
        weight = tra_model.layers[-3].get_weights()
        n_layer.set_weights(weight)
        x = t_model_1.output
        x = n_layer(x)
        t_model_1 = M(inputs=t_model_1.input, outputs=x)
        t_model_1 = reset_model(1, t_model_1)
        t_model_1.summary()
        soure_data_3 = t_model_1.predict(x=train_data['tra1'])
        soure_data_4 = t_model_1.predict(x=train_data['tra2'])
        t_model_1.compile(loss=pearson,
                          optimizer=Nadam(learning_rate=yml['ML']['learningRate'], beta_1=0.9, beta_2=0.999))
        print('--------------- 2 fit pearson te1---------------')
        t_model_1.fit(x=test_data['te1'], y=soure_data_4,
                      shuffle=yml['ML']['shuffle'],
                      batch_size=yml['ML']['batch_size'],
                      epochs=yml['ML']['trainEpoch'],
                      callbacks=callback)
        print('--------------- 2 fit pearson te2---------------')
        t_model_1.fit(x=test_data['te2'], y=soure_data_3,
                      shuffle=yml['ML']['shuffle'],
                      batch_size=yml['ML']['batch_size'],
                      epochs=yml['ML']['trainEpoch'],
                      callbacks=callback)
        ...
        y = dense(y)
        y = sigmoid(y)
        te_model = M(inputs=t_model_1.input, outputs=y)
        # te_model = reset_model(3, te_model)
        # te_model.summary()
        # plot_model(tra_model, show_shapes=True, to_file='/home/xumeiyan/Public/model_image/te_model')
        # te_model.compile(loss='binary_crossentropy',
        #                  optimizer=Nadam(learning_rate=yml['ML']['learningRate'], beta_1=0.9, beta_2=0.999),
        #                  metrics=[keras.metrics.BinaryAccuracy(name='acc'),
        #                            keras.metrics.TruePositives(name='tp'),
        #                            keras.metrics.FalsePositives(name='fp'),
        ...
        #                            keras.metrics.Recall(name='recall'),
        #                            keras.metrics.AUC(name='auc')])
        # te_model.fit(x=train_data['tra1'], y=train_data['tra1_label'],
        #              shuffle=yml['ML']['shuffle'],
        #              batch_size=yml['ML']['batch_size'],
        #              epochs=yml['ML']['trainEpoch'],
        #              class_weight=class_weight_f(yml, train_data['tra1_label']),
        #              callbacks=callback)
        # te_model.fit(x=train_data['tra2'], y=train_data['tra2_label'],
        #              shuffle=yml['ML']['shuffle'],
        #              batch_size=yml['ML']['batch_size'],
        #              epochs=yml['ML']['trainEpoch'],
        #              class_weight=class_weight_f(yml, train_data['tra2_label']),
        ...
        for tra_index, te_index in kf.split(X):
            ...
            new_model = reset_model(3, new_model)
            m = m + 1
            ...
            if not os.path.exists(transfer_best_path):
                ...
                te_model.save(os.path.join(transfer_best_path, 'transfer_Dense_2_400Hz_HA_pearson.h5'))
            else:
                te_model.save(os.path.join(transfer_best_path, 'transfer_Dense_2_400Hz_HA_pearson.h5'))
        ...
        writerxlsx_1(
            path=os.path.join(best_dir, 'val_Dense_400Hz_HA_pearson.xlsx'),
    ...
    writerxlsx_1(
        path=os.path.join('/home/xumeiyan/Public/Reslut/CMANet/Try/202201201_base3DCNN_OpenBMI_400Hz_2class_4_9channel_0.0625_Combined_Attention_1Layer/result',
                          'transfer_Dense_2_val_400Hz_HA_pearson.xlsx'),
