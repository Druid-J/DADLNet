...
if __name__ == '__main__':
    ...
    for sub in sub_all:
        ...
        x = tra_model.output
        x = tf.keras.layers.Reshape((128, 1), input_shape=(128,), name='reshape_relevant')(x)
        x = tf.keras.layers.Conv1D(128, 1, name='Conv_relevant_encoder')(x)
        x = tf.keras.layers.Conv1D(1, 1, name='Conv_relevant_decoder')(x)
        x = tf.keras.layers.Flatten()(x)
        x = tf.keras.layers.Dense(64, activation='elu', name='Dense_relevant_1')(x)
        x = tf.keras.layers.Dense(32, activation='elu', name='Dense_relevant_2')(x)
        ...
        tra_model.fit(x=train_data['tra2'], y=train_data['tra2_label'],
                     ...
                     callbacks=callback)
        t_model = M(inputs=tra_model.input, outputs=tra_model.get_layer(index=44).output)
        t_model = reset_model(1, t_model)
        ...
        for tra_index, te_index in kf.split(X):
            ...
            if not os.path.exists(transfer_best_path):
                ...
                te_model.save(os.path.join(transfer_best_path, 'transfer_Conv_1_400Hz.h5'))
            else:
                te_model.save(os.path.join(transfer_best_path, 'transfer_Conv_1_400Hz.h5'))
        ...
        writerxlsx_1(
            path=os.path.join(best_dir, 'val_Conv_400Hz_1.xlsx'),
    ...
    writerxlsx_1(
        path=os.path.join('/home/xumeiyan/Public/Reslut/CMANet/Try/202201201_base3DCNN_OpenBMI_400Hz_2class_4_9channel_0.0625_Combined_Attention_1Layer/result',
                          'transfer_Conv_1_val_400Hz.xlsx'),
